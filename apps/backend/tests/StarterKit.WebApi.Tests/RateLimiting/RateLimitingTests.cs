...
namespace StarterKit.WebApi.Tests.RateLimiting;

public abstract class RateLimitingTests : WebApiIntegrationTestBase
{
    ...
    public sealed class ApiDefault_Returns429_WhenLimitExceeded : RateLimitingTests
    {
        ...
        public async Task ApiDefault_WhenAuthenticatedLimitExceeded_Returns429WithRetryAfter()
        {
            ...
            for (var i = 0; i < 3; i++)
                await client.GetAsync("/api/clubs");
    ...
    public sealed class ApiDefault_PartitionsByUserId : RateLimitingTests
    {
        ...
        public async Task ApiDefault_TwoDistinctUsers_DoNotShareRateLimitBucket()
        {
            ...
            for (var i = 0; i < 3; i++)
                await clientA.GetAsync("/api/clubs");

            var userAResponse = await clientA.GetAsync("/api/clubs");
            ...
            var userBResponse = await clientB.GetAsync("/api/clubs");
